...
class Ajax extends \BookneticApp\Providers\Core\Controller
{
    ...
    {
        Capabilities::must('settings_calendar');

        if (! Capabilities::tenantCan('settings_calendar')) {
        ...
        $appointmentCardColor = Helper::getOption('calendar_event_color', 'serviceColor');
        $appointmentCardContent = Helper::getOption('calendar_event_content', "");
        $shouldUseDefaultCardStyles = Helper::getOption('should_use_default_styles_calendar_card', 'on') === 'on';

        return $this->modalView('calendar_settings', [
            "shortcodeList" => $shortcodeList,
            "appointmentCardColor" => $appointmentCardColor,
            "appointmentCardContent" => $appointmentCardContent,
            "shouldUseDefaultCardStyles" => $shouldUseDefaultCardStyles
        ]);
    }
    ...
    public function booking_panel_labels_settings()
    {
        ...
        $paramaters[ 'other_translates' ] = [
            ...
            'We\'ve found your account in our system and have automatically filled in your details.' => bkntc__('We\'ve found your account in our system and have automatically filled in your details.'),
    ...
    public function company_settings()
    {
        ...
        $defaultNoImage = Helper::assets("icons/no-photo.svg");

        return $this->modalView('company_settings', [
            'defaultNoImage' => $defaultNoImage,
        ]);
    }
    ...
    public function profile_details()
    {
        ...
        return $this->modalView('profile_details', [ 'tenant' => Permission::tenantInf() ]);
    }
    ...
    public function change_password()
    {
        ...
        return $this->modalView('change_password');
    }
    ...
    public function delete_account()
    {
        ...
        return $this->modalView('delete_account');
    }
    ...
    public function save_profile_details()
    {
        ...
        $fullName = Helper::_post('fullName', '', 'str');
        $email = Helper::_post('email', '', 'email');
        $domain = Helper::_post('domain', '', 'str');

        if (empty($fullName) || empty($email) || empty($domain)) {
            return $this->response(false, bkntc__('Please fill all the required fields correctly'));
        }
        ...
        $currentTenantInf = Tenant::get(Permission::tenantId());

        if ($currentTenantInf->domain != $domain) {
            $otherTenantsWithGivenDomainCount = Tenant::where('domain', $domain)->count();

            if ($otherTenantsWithGivenDomainCount > 0) {
                return $this->response(false, bkntc__('This domain is registered under another user'));
            }

            $updateData['domain'] = $domain;
        }

        if ($currentTenantInf->email != $email) {
            $otherTenantsWithGivenEmailCount = Tenant::where('email', $email)->count();

            if ($otherTenantsWithGivenEmailCount > 0) {
                return $this->response(false, bkntc__('This email is registered under another user'));
            }

            $updateData['email'] = $email;
        }

        Tenant::whereId(Permission::tenantId())->update($updateData);

        if ($currentTenantInf->email != $email) {
            $this->updateWpUserEmail($email);
        }
     ...
     * @throws Exception
     */
    private function updateWpUserEmail($email)
    {
        $user = get_userdata(Permission::tenantInf()->user_id);
        ...
        if (is_wp_error($updated)) {
            throw new Exception(bkntc__('Error updating user: ') . $updated->get_error_message());
        }
    }
    ...
    public function save_change_password()
    {
        ...
        $currentPassword = Helper::_post('currentPassword', '', 'str');
        $newPassword = Helper::_post('newPassword', '', 'str');
        $newPasswordConfirm = Helper::_post('newPasswordConfirm', '', 'str');
        ...
        if ($newPassword !== $newPasswordConfirm) {
            return $this->response(false, bkntc__('The new password and confirmation doesn\'t match'));
        }

        $user = get_userdata(Permission::tenantInf()->user_id);

        if (! $user || ! wp_check_password($currentPassword, $user->data->user_pass, $user->ID)) {
            return $this->response(false, bkntc__('Current password is incorrect'));
        }

        $updated = wp_update_user([
            'ID' => Permission::tenantInf()->user_id,
            'user_pass' => $newPassword
        ]);

        if (is_wp_error($updated)) {
            return $this->response(false, bkntc__('Error updating user: ') . $updated->get_error_message());
        }
    ...
    public function update_profile_picture()
    {
        ...
        if (! isset($_FILES[ 'file' ]) || ! is_string($_FILES[ 'file' ][ 'tmp_name' ])) {
            return $this->response(false);
        }
        ...
        $id = md5(base64_encode(rand(1, 9999999) . microtime(true))) . '.' . $extension;
        $name = Helper::uploadedFile($id, 'Tenants');

        move_uploaded_file($_FILES[ 'file' ][ 'tmp_name' ], $name);

        Tenant::whereId(Permission::tenantId())->update([
            'picture' => $id
        ]);

        return $this->response(true, [
            'url' => Helper::uploadedFileURL($id, 'Tenants')
        ]);
    }
    ...
    public function delete_profile_picture()
    {
        ...
        $picture = Permission::tenantInf()->picture;

        if (empty($picture)) {
            return $this->response(true);
        }

        $filePath = Helper::uploadedFile($picture, 'Tenants');
        ...
        return $this->response(true, [
            'noPhotoUrl' => Helper::noPhoto('Settings')
        ]);
    }

    public function delete_tenant_account()
    {
        if (! Helper::isTenant()) {
            return $this->response(false);
        }

        $tenant = Permission::tenantInf();
    ...
    public function save_general_settings()
    {
        ...
        if (!array_key_exists($default_appointment_status, Helper::getAppointmentStatuses())) {
        ...
        } elseif (Capabilities::tenantCan('remove_branding')) {
    ...
    public function save_advanced_settings()
    {
        ...
        if (! Capabilities::tenantCan('settings_advanced')) {
            return $this->response(false);
        }
    ...
    public function save_calendar_settings()
    {
        ...
        $appointmentCardColor = Helper::_post('appointmentCardColor', 'eventColor', 'string');
        $eventContent = Helper::_post('eventContent', '', 'string');
        $shouldUseDefaultStyling = Helper::_post('shouldUseDefaultStyling', 'on', 'string');

        Helper::setOption('calendar_event_color', $appointmentCardColor);
        Helper::setOption('calendar_event_content', $eventContent);
        Helper::setOption('should_use_default_styles_calendar_card', $shouldUseDefaultStyling);
    ...
    public function save_booking_steps_settings()
    {
        ...
        $customer_identifier                = Helper::_post('customer_identifier', 'email', 'string', [ 'email', 'phone' ]);
        ...
        if ($customer_identifier === 'email' && $set_email_as_required === 'off') {
            return $this->response(false, bkntc__('As a Customer identifier, Email must be a required field!'));
        }

        if ($customer_identifier === 'phone' && $set_phone_as_required === 'off') {
            return $this->response(false, bkntc__('As a Customer identifier, Phone number must be a required field!'));
        }
        ...
        Helper::setOption('customer_identifier', $customer_identifier);
    ...
    public function handle_company_logo_upload()
    {
        Capabilities::must('settings_company');
        ...
        if (!isset($_FILES['company_image']) || !is_string($_FILES['company_image']['tmp_name'])) {
            return $this->response(false, bkntc__('Invalid file upload.'));
        }
        ...
        if (is_uploaded_file($_FILES['company_image']['tmp_name']) && move_uploaded_file($_FILES['company_image']['tmp_name'], $file_name)) {
    ...
    public function handle_company_logo_delete()
    {
        Capabilities::must('settings_company');
        ...
        if (!empty($oldFileName)) {
            ...
            Helper::setOption('company_image', '');
        }
    ...
    public function get_translation()
    {
        ...
        foreach (Bootstrap::$addons as $addon) {
            LocalizationService::loadTextdomain($addon::getAddonSlug());
        }
