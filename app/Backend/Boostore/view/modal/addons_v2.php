...
<div class="row addons_card_wrapper">
    <?php foreach ($data[ 'items' ] as $addon): ?>
        <div class="card_col col-xl-3 col-lg-4 col-md-6">
            <div class="addons_card<?php echo ! empty($addon[ 'is_new' ]) ? ' booknetic_is_new_addon' : '' ?>">
                    ...
                    <div class="card_btns">
                        <?php if ($addon[ 'purchase_status' ] === 'unowned'): ?>
                            <?php if ($addon[ 'in_cart' ]): ?>
                                <a class="btn btn-lg btn-warning view_cart_btn mb-2 mr-2"
                                   href="admin.php?page=<?php echo Helper::getBackendSlug(); ?>&module=cart"> <i
                                            class="fa fa-shopping-cart mr-2"
                                            aria-hidden="true"></i> <?php echo bkntc__('VIEW CART'); ?> </a>
                            <?php else: ?>
                                <button class="btn btn-primary btn-lg mr-2 mb-2 btn-add-to-cart"
                                        data-addon="<?php echo htmlspecialchars($addon[ 'slug' ]); ?>">
                                    <?php echo bkntc__('ADD TO CART'); ?>
                                </button>
                            <?php endif; ?>
                            <?php elseif (! empty($addon[ 'is_installed' ])): ?>
                                <button class="btn btn-outline-danger btn-lg mr-2 mb-2 btn-uninstall"
                                        data-addon="<?php echo htmlspecialchars($addon[ 'slug' ]); ?>">
                                    <?php echo bkntc__('UNINSTALL'); ?>
                                </button>
                        <?php elseif ($addon[ 'purchase_status' ] === 'owned'): ?>
                            <button class="btn btn-success btn-lg mr-2 mb-2 btn-install"
                                    data-addon="<?php echo htmlspecialchars($addon[ 'slug' ]); ?>">
                        ...
                        <?php elseif (! empty($addon[ 'error_message' ])): ?>
                            <button class="btn btn-outline-danger btn-lg mr-2 mb-2 do_tooltip" data-placement="bottom"
                        ...
                        <a class="btn btn-primary btn-lg btn-info mb-2"
                           href="admin.php?page=<?php echo Helper::getBackendSlug(); ?>&module=boostore&action=details&slug=<?php echo $addon[ 'slug' ]; ?>">
