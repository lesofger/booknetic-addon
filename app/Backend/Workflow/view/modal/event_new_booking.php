 ...
 */
?>
<script type="application/javascript" src="<?php echo Helper::assets('js/event_new_booking.js', 'workflow')?>"></script>
            ...
            <div class="form-group col-md-12">
                ...
                <select class="form-control" id="input_locations" multiple>
                    ...
                    foreach ($parameters['locations'] as $location) {
                        echo '<option value="' . (int)$location['id'] . '" selected>' . htmlspecialchars($location['name']) . '</option>';
                    }
?>
                </select>
            </div>
...
foreach ($parameters['services'] as $service) {
    echo '<option value="' . (int)$service['id'] . '" selected>' . htmlspecialchars($service['name']) . '</option>';
}
?>
                </select>
            </div>
...
foreach ($parameters['staffs'] as $staff) {
    echo '<option value="' . (int)$staff['id'] . '" selected>' . htmlspecialchars($staff['name']) . '</option>';
}
?>
                </select>
            </div>
...
foreach ($parameters['statuses'] as $status) {
    echo '<option value="' . $status[0] . '" selected>' . htmlspecialchars($status[1]) . '</option>';
}
?>
                </select>
            </div>
