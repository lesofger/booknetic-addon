...
$firstName = htmlspecialchars($customer->getFirstName());
...
<div class="form-row">
    <div class="form-group col-md-6">
        <label for="input_email"><?php echo bkntc__('Email') ?> <?php echo $parameters->isEmailRequired() ? '<span class="required-star">*</span>' : '' ?></label>
        <input type="text" class="form-control" id="input_email" placeholder="example@gmail.com" value="<?php echo htmlspecialchars($customer->getEmail()) ?>">
        <input type="hidden" id="customer_original_email" value="<?php echo htmlspecialchars($customer->getEmail()) ?>">
    </div>
    <div class="form-group col-md-6">
        <label for="input_phone"><?php echo bkntc__('Phone') ?> <?php echo $parameters->isPhoneRequired() ? '<span class="required-star">*</span>' : '' ?></label>
        <input type="tel" class="form-control" id="input_phone" value="<?php echo htmlspecialchars($customer->getPhoneNumber()) ?>" data-country-code="<?php echo Helper::getOption('default_phone_country_code', '') ?>">
    </div>
</div>
    ...
    <div class="form-row">
        <div class="form-group col-md-6">
            ...
            <div class="form-control-checkbox">
                ...
                <div class="fs_onoffswitch <?php if (!$parameters->hasWpUser()) {
                    echo 'disabled';
                } ?>">
                    <input type="checkbox" <?php if (!$parameters->hasWpUser()) {
                        echo 'disabled';
                    } ?> class="fs_onoffswitch-checkbox" id="input_allow_customer_to_login" <?php echo ($customer->getUserId() > 0 ? ' checked' : '') ?>>
        ...
        <?php if (!Helper::isSaaSVersion()) : ?>
            <div class="form-group col-md-6" data-hide="allow_customer_to_login">
                <select class="form-control" id="input_wp_user_use_existing">
                    <option value="yes" <?php echo ($customer->getUserId() > 0 ? ' selected' : '') ?>><?php echo bkntc__('Use existing WordPress user') ?></option>
            ...
            <div class="form-group col-md-6" data-hide="existing_user">
                ...
                <select class="form-control" id="input_wp_user">
                    ...
                    foreach ($parameters->getUsers() as $user):
                        if (! (($userData = get_userdata($user['ID'])) && in_array('booknetic_staff', $userData->roles))):
                            ?>
                        <option value="<?php echo (int)$user['ID'] ?>" <?php echo ($user['ID'] == $customer->getUserId() ? ' selected' : '') ?>
                        ...
                        ><?php echo htmlspecialchars($user['display_name']) ?></option>
                    <?php
                        endif;
                    endforeach;
            ?>
        ...
        <div class="form-group col-md-6" data-hide="create_password">
            ...
            <input type="text" <?php if (!$parameters->hasWpUser()) {
                echo 'disabled';
            } ?> class="form-control" id="input_wp_user_password" placeholder="*****">
        </div>
    </div>
<?php endif; ?>
...
<div class="form-row">
    <div class="form-group col-md-6">
        ...
        <select id="input_gender" class="form-control" placeholder="<?php echo bkntc__('Gender') ?>">
            <option disabled <?php echo (empty($customer->getGender()) ? ' selected' : '') ?>>
                <?php echo bkntc__('Select gender') ?>
            </option>
            <option value="male" <?php echo ($customer->getGender() === 'male' ? ' selected' : '') ?>>
                <?php echo bkntc__('Male') ?>
            </option>
            <option value="female" <?php echo ($customer->getGender() === 'female' ? ' selected' : '') ?>>
    ...
    <div class="form-group col-md-6">
        ...
        <div class="inner-addon left-addon">
            ...
            <input data-date-format="<?php echo (htmlspecialchars(Helper::getOption('date_format', 'Y-m-d'))) ?>" type="text" class="form-control" id="input_birthday" value="<?php echo (empty($customer->getBirthdate()) ? '' : Date::convertDateFormat($customer->getBirthdate())) ?>">
        </div>
    </div>
</div>
...
<div class="form-row">
    <div class="form-group col-md-12">
        ...
        <textarea id="input_note" class="form-control"><?php echo htmlspecialchars($customer->getNotes() ?? '') ?></textarea>
    </div>
</div>